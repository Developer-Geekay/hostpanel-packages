"""
hostpanel-maild — the mail package API (Postfix & Dovecot).

An independent HTTP API owning mail domain, virtual mailbox, alias, DKIM, and
queue lifecycle. It runs as its own unprivileged system user (`hp-mail`) and binds
127.0.0.1:9113 only.
...
from hostpanel_maild import routes

_log = logging.getLogger(__name__)

#: Default location of this package's manifest
...
def main() -> None:
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9113"))
    host = os.environ.get("HP_PACKAGE_HOST", "127.0.0.1")
    if host not in ("127.0.0.1", "localhost", "::1"):
        raise SystemExit(
            f"HP_PACKAGE_HOST={host!r} is not a loopback address. This service "
            f"exposes privileged mail operations and must not be "
            f"reachable off-host."
        )

    _log.info("hostpanel-maild starting on %s:%d", host, port)
