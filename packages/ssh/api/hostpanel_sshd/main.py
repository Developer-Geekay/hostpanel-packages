"""
hostpanel-sshd — the ssh package API.

An independent HTTP API owning exactly one domain: SSH access management —
authorized keys and per-user password-authentication policy. It runs as its
own unprivileged system user (`hp-ssh`) and binds 127.0.0.1 only.

Its entire privilege is one sudoers line:

    hp-ssh ALL=(root) NOPASSWD: /opt/hostpanel/packages/ssh/ops/hp-ssh *

One script. It cannot edit ~/.ssh/authorized_keys, write to
/etc/ssh/sshd_config.d or reload sshd directly — only through that script's
validation.

It holds NO state and never opens the database. Everything it needs arrives as
request parameters.

Run:  python -m hostpanel_sshd
"""
...
from hostpanel_sshd import routes

_log = logging.getLogger(__name__)

#: Default location of this package's manifest, relative to this file:
#: packages/ssh/api/hostpanel_sshd/main.py -> packages/ssh/manifest.json
...
def main() -> None:
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9101"))
    ...
    _log.info("hostpanel-sshd starting on %s:%d", host, port)
