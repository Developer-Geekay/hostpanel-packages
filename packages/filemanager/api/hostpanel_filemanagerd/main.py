"""
hostpanel-filemanagerd — the filemanager package API.

Runs as its own unprivileged system user (`hp-filemanager`) and binds 127.0.0.1 only.
...
from hostpanel_filemanagerd import routes
...
def main() -> None:
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9104"))
    host = os.environ.get("HP_PACKAGE_HOST", "127.0.0.1")
    if host not in ("127.0.0.1", "localhost", "::1"):
        raise SystemExit(
            f"HP_PACKAGE_HOST={host!r} is not a loopback address. This service "
            f"exposes privileged file operations and must not be reachable off-host."
        )

    _log.info("hostpanel-filemanagerd starting on %s:%d", host, port)
