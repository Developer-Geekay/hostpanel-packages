/**
 * Shared presentation kit for storage package UI.
 *
 * Provides HostPanel design system tokens and presentation components:
 * MONO, MicroLabel, Panel, Field, Readout, Dot, LogPane, appendEvent.
 */
...
/** A small uppercase label — the panel structural voice. */
...
/** A coloured dot indicating status. */
...
export function appendEvent(lines: Line[], event: any): Line[] {
  switch (event?.kind) {
    case "log":
      return [...lines, { stream: event.stream ?? "stdout", text: event.line ?? "" }];
    case "error":
...
/** Terminal view for streamed ops execution logs. */
