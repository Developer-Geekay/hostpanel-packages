"""
Unit tests for hostpanel-apached API service.
"""
...
verb="$1"; shift
case "$verb" in
  engine-status)     printf '{"ok":true,"engine":"apache","service":"apache2","status":"running","active":true,"pid":1234,"uptime":"1h","version":"Apache/2.4.58","port":8088,"mpm":"event","workers":5}
' ;;
...
' ;;
  engine-test)       printf '{"ok":true,"valid":true,"output":"Syntax OK"}
' ;;
  engine-config-get) printf '{"ok":true,"path":"/opt/hostpanel/etc/apache/apache2.conf","content":"ServerRoot /etc/apache2"}
' ;;
  engine-config-set) printf '{"ok":true,"updated":true,"path":"/opt/hostpanel/etc/apache/apache2.conf"}
' ;;
  engine-logs)       printf '{"ok":true,"log_type":"error","lines":["line 1","line 2"]}
' ;;
  engine-install)    printf '{"ok":true,"engine":"apache","status":"installed"}
' ;;
  vhost-list)        printf '{"ok":true,"vhosts":[{"domain":"test.com","file":"test.com.conf","enabled":true,"port":80,"ssl":false,"root":"/var/www/html"}]}
' ;;
  vhost-get)         printf '{"ok":true,"domain":"%s","content":"<VirtualHost *:80></VirtualHost>"}
' "$1" ;;
...
' "$1" ;;
  module-list)       printf '{"ok":true,"modules":[{"name":"rewrite","enabled":true}]}
' ;;
  module-enable)     printf '{"ok":true,"module":"%s","enabled":true}
' "$1" ;;
  module-disable)    printf '{"ok":true,"module":"%s","enabled":false}
' "$1" ;;
...
def svc(tmp_path, monkeypatch):
    script = tmp_path / "hp-apache-fake"
    ...
    from hostpanel_apached import main as apached

    app = apached.create_app(M.load(MANIFEST))
...
def test_health_needs_no_token(svc):
    ...
    assert resp.json() == {"package": "apache", "version": "3.0.0", "ok": True}
...
def test_vhost_management(svc):
    ...
    assert svc.client.post("/vhosts", json={"domain": "test.com", "content": "<VirtualHost></VirtualHost>"}).status_code == 200
...
def test_modules(svc):
    ...
    assert svc.client.post("/modules/enable", json={"module": "rewrite"}).status_code == 200
    assert svc.client.post("/modules/disable", json={"module": "rewrite"}).status_code == 200
