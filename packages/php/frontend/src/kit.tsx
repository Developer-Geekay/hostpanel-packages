...
function cleanTerminalLine(raw: string): string {
  if (!raw) return "";
  const stripped = raw.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, "");
  if (stripped.includes("\r")) {
    const parts = stripped.split("\r").filter((p) => p.length > 0);
...
export function appendEvent(lines: Line[], event: any): Line[] {
  switch (event?.kind) {
    case "log": {
      const text = cleanTerminalLine(event.line ?? "");
      if (!text && !event.line) return lines;
      if (text.trim().startsWith('{"ok":') || text.trim().startsWith('{"error":') || text.trim().startsWith('{"status":') || text.trim().startsWith('{"package":')) {
        return lines;
      }
      const isProgress = (event.line ?? "").includes("\r") || /^\s*\d+K\s+\.+|\d+%\s+[\d.]+[KMGTB]/i.test(text);
      if (isProgress && lines.length > 0 && lines[lines.length - 1].stream === (event.stream ?? "stdout")) {
        const last = lines[lines.length - 1];
        if (last.text.includes("%") || /^\s*\d+K|\d+%/i.test(last.text)) {
          const next = [...lines];
          next[next.length - 1] = { stream: event.stream ?? "stdout", text };
          return next;
        }
      }
    ...
    case "error":
      return [...lines, { stream: "stderr", text: cleanTerminalLine(event.message ?? "failed") }];
    case "result":
      return [
        ...lines,
        {
