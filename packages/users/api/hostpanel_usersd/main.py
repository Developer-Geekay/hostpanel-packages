"""
hostpanel-usersd — the users package API.

An independent HTTP API owning exactly one domain: Linux account lifecycle. It
runs as its own unprivileged system user (`hp-users`) and binds 127.0.0.1 only.

Its entire privilege is one sudoers line:

    hp-users ALL=(root) NOPASSWD: /opt/hostpanel/packages/users/ops/hp-users *

One script. It cannot invoke useradd, userdel or chpasswd directly — only through
that script's validation. Compare v2, where the equivalent grant is
`useradd *` + `usermod *` + `rm -rf *` held by the whole panel.

It holds NO state and never opens the database. Everything it needs arrives as
request parameters.

Run:  python -m hostpanel_usersd
"""
...
from hostpanel_usersd import routes

_log = logging.getLogger(__name__)

#: Default location of this package's manifest, relative to this file:
#: packages/users/api/hostpanel_usersd/main.py -> packages/users/manifest.json
...
def main() -> None:
    ...
    the network — note the v2 storage service does bind 0.0.0.0, but that is an
    S3 endpoint meant for external clients and is not the pattern here.
    """
    import uvicorn
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9100"))
    ...
    _log.info("hostpanel-usersd starting on %s:%d", host, port)
