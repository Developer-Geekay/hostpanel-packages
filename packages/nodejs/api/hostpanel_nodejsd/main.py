"""
hostpanel-nodejsd — the Node.js package API daemon.

An independent HTTP API owning Node.js application lifecycles, runtime versions,
and reverse proxy port allocation. Binds 127.0.0.1:9111 only.

Privileged operations route strictly through /opt/hostpanel/packages/nodejs/ops/hp-nodejs
via sudo.

Run: python -m hostpanel_nodejsd
"""
...
from hostpanel_nodejsd import routes
...
def main() -> None:
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9111"))
    ...
    _log.info("hostpanel-nodejsd starting on %s:%d", host, port)
