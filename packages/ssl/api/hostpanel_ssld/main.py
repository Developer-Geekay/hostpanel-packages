"""
hostpanel-ssld — the SSL / TLS and ACME certificate package API.

Runs as unprivileged user `hp-ssl`, binds 127.0.0.1:9112 only.
All operations route through /opt/hostpanel/packages/ssl/ops/hp-ssl via sudo.

Run: python -m hostpanel_ssld
"""
...
from hostpanel_ssld import routes
...
def main() -> None:
    ...
    port = int(os.environ.get("HP_PACKAGE_PORT", "9112"))
    ...
    _log.info("hostpanel-ssld starting on %s:%d", host, port)
