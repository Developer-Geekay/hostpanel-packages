"""
hostpanel-ftpd — the ftp package API.

An independent HTTP API owning exactly one domain: pure-ftpd virtual FTP
account lifecycle. It runs as its own unprivileged system user (`hp-ftp`) and
...
One script. It cannot invoke `pure-pw` directly — only through that script's
validation.

It holds NO state and never opens the database. Everything it needs arrives as
request parameters.

Run:  python -m hostpanel_ftpd
"""
...
from hostpanel_ftpd import routes

_log = logging.getLogger(__name__)

#: Default location of this package's manifest, relative to this file:
#: packages/ftp/api/hostpanel_ftpd/main.py -> packages/ftp/manifest.json
...
def main() -> None:
    ...
    Binds loopback ONLY. Binding 0.0.0.0 would expose privileged FTP account
    ...
    if host not in ("127.0.0.1", "localhost", "::1"):
        ...
        raise SystemExit(
            ...
            f"exposes privileged FTP account operations and must not be "
            f"reachable off-host."
        )

    _log.info("hostpanel-ftpd starting on %s:%d", host, port)
